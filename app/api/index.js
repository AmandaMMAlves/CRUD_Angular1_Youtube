...
api.busca = function(req, res) {
   db.findOne({_id: req.params.musicaId }, function(err, doc) {
        if (err) return console.log(err);
        res.json(doc);
    });
};
...
api.atualiza = function(req, res) {
    ...
    db.update({_id : req.params.musicaId }, req.body, function(err, numReplaced) {
        if (err) return console.log(err);
...
api.remove = function(req, res) {
    ...
    db.remove({ _id: req.params.musicaId }, {}, function (err, numRemoved) {
        if (err) return console.log(err);
